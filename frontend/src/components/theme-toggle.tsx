...
export function ThemeToggle(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('fweep-theme', theme);
  }, [theme]);
    ...
    >
      {theme === 'light' ? (
        <svg width="20" height="20" viewBox="0 0 640 640" fill="currentColor" aria-hidden="true">
          <path d={MOON_SOLID_FULL_PATH} />
        </svg>
      ) : (
        <svg width="20" height="20" viewBox="0 0 640 640" fill="currentColor" aria-hidden="true">
          <path d={SUN_SOLID_FULL_PATH} />
        </svg>
      )}
    </button>
  );

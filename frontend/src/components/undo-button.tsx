...
export function UndoButton(): React.JSX.Element {
  ...
  return (
    <button
      ...
      onClick={undo}
      aria-label="Undo"
      aria-keyshortcuts={UI_SHORTCUTS.undo.ariaKeyShortcuts}
      data-shortcut={UI_SHORTCUTS.undo.display}
      title={getShortcutTitle('Undo', UI_SHORTCUTS.undo)}
      type="button"
      disabled={!canUndo}
    >
      <svg width="20" height="20" viewBox="0 0 640 640" fill="currentColor" aria-hidden="true">
        <path d={ARROW_ROTATE_LEFT_PATH} />
      </svg>
    </button>
  );
}
